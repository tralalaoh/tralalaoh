...
spath_database_path = xbmcvfs.translatePath(
    "special://profile/addon_data/script.littleduck.helper/spath_cache.db"
)

search_history_xml = "script-littleduck-search_history"
...
class SPaths:
    ...
    def search_input(self, search_type=None, search_term=None):
        """
        Enhanced search_input that supports custom search paths.
        search_type can be 'movie' or 'tvshow' for Custom Search provider.
        """
        ...
        if search_term is None or not search_term.strip():
            ...
            xbmc.executebuiltin("Skin.SetBool(LiveSearchActive)")
            keyboard = xbmc.Keyboard("", prompt, False)
            keyboard.doModal()
            xbmc.executebuiltin("Skin.Reset(LiveSearchActive)")
        ...
        # Open search window if coming from home
        if xbmcgui.getCurrentWindowId() == 10000:
            xbmc.executebuiltin("ActivateWindow(1121)")
        
        # Set search string skin properties BEFORE updating history XML,
        # so SearchInput is already set when ReloadSkin() executes inside
        # make_search_history_xml and the containers load with the query.
        ...
        # Update search history XML (queues ReloadSkin — strings already set above)
        if xbmcgui.getCurrentWindowId() == 10000:
    ...
    def re_search(self):
        ...
        self.search_input(None, search_term)
