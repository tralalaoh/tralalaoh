...
def widget_monitor(list_id):
    ...
    while not monitor.abortRequested():
        ...
        if window_id not in [10000, 11121]:
            ...
            stack_label_control = None
            for _suffix in ("666", "667", "668", "669", "670", "671", "672"):
                try:
                    stack_label_control = window.getControl(int(stack_id + _suffix))
                    break
                except Exception:
                    pass
        ...
        if list_id != str(window.getFocusId()):
            break
        last_path = window.getProperty("littleduck.%s.path" % list_id)
        ...
        while not monitor.abortRequested() and countdown >= 0 and switch_widget:
            ...
            if display_delay:
                stack_label_control.setLabel(
                    "Loading [COLOR accent_color][B]{label}[/B][/COLOR] in [B]{secs:.2f}[/B] seconds".format(
                        label=widget_label, secs=countdown
                    )
                )
        if switch_widget:
            ...
            stack_label_control.setLabel(
                window.getProperty("littleduck.%s.label" % list_id)
            )
            monitor.waitForAbort(0.25)
    try:
        del monitor
